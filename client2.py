...
def train():
  try:
    ...
    # Take only 100 rows for training
    X_train = X_train[:100]
    y_train = y_train[:100]
    # Partition data
    ...
    class FlowerClient(fl.client.NumPyClient):
      ...
      def on_fit_end(self, config):
        # Log weights after training is completed
        weights = model.coef_.tolist()
        logging.info(f"Round {config['server_round']}: Weights - {weights}")
        # Return weights as JSON response
        return jsonify({"weights": weights})

    # Start Flower client
    fl.client.start_numpy_client(server_address="localhost:8080", client=FlowerClient())

    # Training now returns weights as JSON
